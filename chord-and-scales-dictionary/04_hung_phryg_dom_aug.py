...
def build_diatonic_chords(pitches, chord_type, scale_type):
    """Build diatonic chords of specified type from scale pitches, with all inversions."""
    chords_by_degree = {}
    scale_length = len(pitches)  # 7 for heptatonic, 6 for hexatonic (augmented)
    
    for degree in range(scale_length):  # Iterate through all scale degrees
        if chord_type == 'suspended':
            # Sus4: degrees 0, 3, 4 from root degree (mod scale_length)
            degree_indices = [degree % scale_length, (degree + 3) % scale_length, (degree + 4) % scale_length]
        elif chord_type == 'seventh':
            # Seventh: stack every other degree (0,2,4,6) mod scale_length
            degree_indices = [(degree + i * 2) % scale_length for i in range(4)]
        elif chord_type == 'ninth':
            # Ninth: seventh + 9th (2nd degree + octave) mod scale_length
...
print("Select scale type (once): 1=Hungarian Minor, 2=Phrygian Dominant, 3=Augmented")
scale_choice = input("Enter choice (1/2/3): ").strip()
scale_type_map = {'1': 'hungarian_minor', '2': 'phrygian_dominant', '3': 'augmented'}
scale_names = {
...
while True:
    ...
    try:
        ...
        chords = build_diatonic_chords(scale_pitches, chord_type, scale_type)
