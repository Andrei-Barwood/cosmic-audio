...
def generate_hungarian_minor(root_note):
    """Generate Hungarian minor scale pitches from given root with explicit octaves."""
    try:
...
print("Hungarian Minor Scale Generator with Chords")
...
while True:
    ...
    if not user_input or user_input.lower() == 'quit':
        ...
        break
    try:
        scale_pitches = generate_hungarian_minor(user_input)
        scale_notes = [p.nameWithOctave for p in scale_pitches]
        print(f"{user_input} Hungarian Minor Scale: {' - '.join(scale_notes)}\n")
